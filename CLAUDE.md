...
### `tape` — 無端テープ（マージソート3テープ / ノームソート1テープ用）
```python
_tape(id, cells, head, label="", color="#4472C4", weight=1,
      hl=None, sorted_upto=None, swap=None, reserve_arc=False, head_label=None)
```
- `hl`: `{インデックス: "色"}` で個別セルをハイライト
- `sorted_upto`: このインデックス未満を「整列確定」（薄緑）で表示
- `swap`: `{"a": idx, "b": idx, "phase": "before"|"after"}` — セルの**上**に交換アーク（⇄）を描く
  - `before` = オレンジ（これから交換）/ `after` = シアン（交換済み）
- `reserve_arc`: `True` でセル上に注釈スペース・セル下に添字ラベルを確保し、
  セルサイズを「全セル + 前後の余白」が収まる範囲まで拡大する（単体テープ用）
...
| ソート | マージソート(反復), マージソート(再帰), マージソート(3テープ), ヒープソート, バケツソート, 基数ソート(LSD), ノームソート(1テープ) |
...
| ファイル | バージョン |
|---|---|
| `static/js/app.js` | **v18** |
| `static/js/array_canvas.js` | **v20** |
| `static/js/ws_client.js` | **v2** |
| `static/css/style.css` | **v5** |
